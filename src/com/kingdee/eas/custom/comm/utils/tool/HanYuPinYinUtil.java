...
public class HanYuPinYinUtil {
	...
	private static HanYuPinYinUtil characterParser;  
	...
	public static HanYuPinYinUtil getInstance() {  
		if(characterParser == null){  
			characterParser = new HanYuPinYinUtil();  
		}  
	...
	private int getChsAscii(String chs) {  
		int asc = 0;  
		try {  
			byte[] bytes = chs.getBytes("GB18030-2000");  
	 ...
	 * @return 拼音字符串 
	 */  
	public String getPinYinSpellingHead(String chs) {  
		String key, value;  
		buffer = new StringBuilder();  
		for (int i = 0; i < chs.length(); i++) {  
			key = chs.substring(i, i + 1);  
	 ...
	 * @return 拼音字符串 
	 */  
	public String getPinYinSpellingHeadBig(String chs) {  
		String key, value;  
		buffer = new StringBuilder();  
		for (int i = 0; i < chs.length(); i++) {  
			key = chs.substring(i, i + 1);  
			...
			buffer.append(value.substring(0, 1).toUpperCase());  
		}  
		return buffer.toString();  
	}  
	 ...
	 * @return 拼音字符串 
	 */  
	public String getPinYinSpellingBigHead(String chs) {  
		String key, value;  
		buffer = new StringBuilder();  
		for (int i = 0; i < chs.length(); i++) {  
			key = chs.substring(i, i + 1);  
			...
			buffer.append(value.substring(0, 1).toUpperCase()).append(value.substring(1, value.length()).toLowerCase());  
		}  
		return buffer.toString();  
	}  
