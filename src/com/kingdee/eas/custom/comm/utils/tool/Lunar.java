...
/*************************************************************
 * Lunar.java,此为农历类，包括农历的算法，算出对应公历的农历日期和农历节日。
*************************************************************/
...
import com.kingdee.eas.util.client.MsgBox;
   ...
   public class Lunar {
     ...
     private int month;
     private int day;
     private int dayofmonth;//记录一个月的天数
     private boolean leap;
//     private String lftv;
      final static String chineseNumber[] = {"一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"};
     static SimpleDateFormat chineseDateFormat = new SimpleDateFormat("yyyy年MM月dd日");//日期模式
      ...
      *********************************************/
      public Lunar(Calendar cal) {
         ...
         month = iMonth;
         day = offset + 1;
         dayofmonth=monthDays(year, month);//得到这一月的总天数
      ...
      public String toString() {  //////显示农历格式,包括年月日,闰月,生肖,天干地支等.
         return year + "年" + (leap ? "闰" : "") + chineseNumber[month - 1] + "月" + getChinaDayString(day);
     }
      public String LunarFestival(){
    	  
    	  Festival ftv=new Festival();//调用节日类的显示农历节日的函数,判断农历12月的天数.
      ...
      public static void getLunarDate(Date date) throws ParseException {
  		...
  		System.out.println(year);// 得到当前时间的年份
  		System.out.println(month);// 得到当前时间的月份
  		System.out.println(day);// 得到当前时间的日
...
//  		String s = ft.showSFtv(month, day);// 公历月份和日期都可以获得
//  		String s1 = ft.showSFtv(11, 11);
//  		System.out.println(s + "+++++++++++++==");
//  		System.out.println("11月11日是" + s1 + "++++++++");
...
//  				.println(lunar.animalsYear() + "++++" + lunar.LunarFestival());
  		...
  		MsgBox.showInfo(l + "&&&&&&&&&");
  		MsgBox.showInfo(lunar.cyclical() + "年");// 计算输出阴历年份
  		MsgBox.showInfo(lunar.toString());// 计算输出阴历日期
  		MsgBox.showInfo(lunar.animalsYear());// 计算输出属相
//  		MsgBox.showInfo(new java.sql.Date(today.getTime().getTime()));// 输出阳历日期
